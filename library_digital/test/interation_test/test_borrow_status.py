...
# TEST XEM TRẠNG THÁI ĐƠN MƯỢN
def test_borrow_status_success(client):
    response = client.get('/user/1/borrow-status')
    ...
    assert "01/01/2026 00:00" in html

# TEST DANH SÁCH TRỐNG
def test_borrow_status_user_not_found(client):
    response = client.get('/user/999/borrow-status')
